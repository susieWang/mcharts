...
module.exports = function(config) {
    console.log("config***"+config.time);
	const configer = {
	...
	return (req, res ,next) => {
	    // 这个函数赋值给了express use，请求发出后，这个函数会被执行
        console.log("perfomance return");
	    ...
	    if (url.pathname == '/action') {
            ...
            console.log("hset **** "+urlData);
            //把 拼接好的url数据存放到redis 里面
...
function saveData(config) {
	return () => {
        console.log("perfomance saveData");
        // redis 数据库存储
        ...
        //从redis中把这些数据取出来，然后存放到指定文件中
        client.hkeys("hash key", function (err, replies) {
            console.log(replies.length + " replies:");
            replies.forEach(function (reply, i) {
                console.log("replies    " + i + ": " + reply);
                arr.push(reply);
            });
            if (arr.length > 0) {
                arr.push('\n');
